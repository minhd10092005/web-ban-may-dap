...
const AdminManager = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await adminApi.getAll(filters);
  ...
  const handleCreate = async () => {
    ...
    } catch { toast.error("Lỗi khi thêm Admin"); }
  };

  const handleUpdate = async () => {
    try {
        await adminApi.update(editItem.id, formData);
        toast.success("Cập nhật thành công!");
        setModalOpen(false);
        loadData();
    } catch { toast.error("Lỗi khi cập nhật Admin"); }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (dataList.length === 1 && filters.pageNumber > 1)
  ...
  const openAdd  = ()  => { setEditItem(null); setFormData({fullName:"", email:"", phoneNumber:"", password:""}); setModalOpen(true); };
  const openEdit = (item) => { setEditItem(item); setFormData({fullName: item.fullName, email: item.email, phoneNumber: item.phoneNumber, password:""}); setModalOpen(true); };
    ...
    <div className="page-wrapper">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {meta.totalCount > 0 ? `${meta.totalCount} admin trong hệ thống` : "Chưa có dữ liệu"}
          </p>
        </div>
      ...
      <div className="toolbar">
        <div className="search-wrap">
          ...
          <input className="search-input" type="text" placeholder="Tìm tên hoặc email..." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} />
        </div>
      ...
      <div className="table-container">
        <div className="table-scroll">
          {loading ? (
            <div className="state-box"><div className="spinner" /><p className="state-text">Đang tải dữ liệu...</p></div>
          ) : dataList.length === 0 ? (
            <div className="state-box">
              <div className="state-icon">📭</div><p className="state-text">Không có dữ liệu</p>
            </div>
          ) : (
              ...
              <tbody>
                {dataList.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="col-name">{item.fullName}</td>
                    <td>{item.email}</td>
                    <td>{item.phoneNumber}</td>
                    <td className="col-actions">
                      <div className="row-actions">
                        <button className="btn btn-sm btn-ghost" onClick={() => openEdit(item)} title="Sửa"><IconEdit /> Sửa</button>
                        <button className="btn btn-sm btn-danger" onClick={() => setDeleteTarget(item)} title="Xóa"><IconTrash /> Xóa</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setModalOpen(false)}>
            <div className="modal" role="dialog" aria-modal="true">
                <div className="modal-header">
                    <h2 className="modal-title">{editItem ? "Cập nhật Admin" : "Thêm Admin Mới"}</h2>
                ...
                <form onSubmit={onSubmit}>
                  <div className="modal-body">
                      <div className="field">
                          ...
                          <input type="text" className="field-input" required value={formData.fullName} onChange={(e)=>setFormData({...formData, fullName: e.target.value})} placeholder="Nhập họ tên" autoFocus />
                      </div>
                      <div className="field">
                          <label className="field-label">Email <span className="req">*</span></label>
                          <input type="email" className="field-input" required value={formData.email} onChange={(e)=>setFormData({...formData, email: e.target.value})} placeholder="Nhập email" />
                      </div>
                      <div className="field">
        ...
        <div className="modal-overlay" onClick={() => !deleting && setDeleteTarget(null)}>
          <div className="confirm-dialog" onClick={(e) => e.stopPropagation()}>
            <div className="confirm-content">
              <h3>Xác nhận xóa</h3>
              <p>Bạn có chắc muốn xóa <strong>{deleteTarget.fullName}</strong>?</p>
            </div>
